<?php
  ...
  try {
    ...
    $access_options = $OBJ_ACCESO_OPCION->getPermitsOptions($_SESSION['id_persona'],printCodeOption("websocio"));
    ...
    if (empty(trim($titulo))) {
      throw new Exception("Campo obligatorio : Nombre.");
    }
    ...
    if ($flag_imagen=="1") {
      ...
      if (isset($_FILES["src_imagen"])){
        ...
        if ($tipo != 'image/jpg' && $tipo != 'image/jpeg'){
          ...
          $copia = imagecreatetruecolor(127,127);
          imagecopyresampled($copia,$original,0,0,0,0,127,127,$width,$height);
    ...
    switch ($accion) {
      case 'add':
        $VD = $OBJ_SOCIO->insert($id,$titulo,$descripcion,$estado,$flag_imagen,$src_imagen);
        break;
      case 'edit':
        $VD = $OBJ_SOCIO->update($id,$titulo,$descripcion,$estado,$flag_imagen,$src_imagen);
        break;
      default:
