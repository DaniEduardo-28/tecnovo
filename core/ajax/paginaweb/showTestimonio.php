<?php
  ...
  try {
    ...
    $DataCantidad = $OBJ_TESTIMONIO->getCount("all","3");

    if ($DataCantidad["error"]=="NO") {

      $cantidad = $DataCantidad["data"][0]["cantidad"];
      $Resultado = $OBJ_TESTIMONIO->show("all","3",$offset,$limit);
      ...
      foreach ($Resultado["data"] as $key) {
        ...
        $desc = strlen($key['descripcion'])>40 ? substr($key['descripcion'],0,40) : $key['descripcion'];
        $desc .= '...';
