<?php
	...
	class ClassTipoMascota extends Conexion {
		...
		public function insert($name_tipo,$estado) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Mascota", "Insertar"]);
			if ($stmt->rowCount() == 0) {
		...
		public function update($id_tipo_mascota,$name_tipo,$estado) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Mascota", "Actualizar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
		...
		public function delete($id_tipo_mascota) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Mascota", "Eliminar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
