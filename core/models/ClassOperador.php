<?php
	...
	class ClassOperador extends Conexion {
		...
		/* public function addPromocion($id_operador,$titulo,$descripcion,$fecha_inicio,$fecha_fin,$src_imagen) {
			$conexionClass = new Conexion();
			$conexion = $conexionClass->Open();
			$VD ;
			try {
				...
				$sql = "INSERT INTO tb_promocion (`id_promocion`, `id_operador`, `titulo`, `descripcion`, `precio`, `fecha_inicio`, `fecha_fin`, `src_imagen`, `estado`) VALUES ";
				$sql .= "(";
				$sql .= "(SELECT CASE COUNT(c.id_promocion) WHEN 0 THEN 1 ELSE (MAX(c.id_promocion) + 1) end FROM `tb_promocion` c),";
				$sql .= "?,?,?,?,?,?,?,?";
				$sql .= ")";
				$stmt = $conexion->prepare($sql);
				$stmt->execute([$id_operador,$titulo,$descripcion,0,$fecha_inicio,$fecha_fin,$src_imagen,"1"]);
				if ($stmt->rowCount()==0) {
...
/* 
		public function showPromocionesOperador($id_operador) {
			...
			try {
				...
				$sql = "SELECT * FROM tb_promocion WHERE id_operador = ? AND estado = '1' AND fecha_fin >= ?";
				$stmt = $conexion->prepare($sql);
				$stmt->execute([$id_operador,$fecha_actual]);
				$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
