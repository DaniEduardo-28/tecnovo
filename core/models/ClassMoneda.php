<?php
	...
	class ClassMoneda extends Conexion {
		...
		public function insert($id_moneda,$estado,$flag_principal,$name_moneda,$cod_sunat,$signo,$abreviatura) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Moneda", "Insertar"]);
			if ($stmt->rowCount() == 0) {
		...
		public function update($id_moneda,$estado,$flag_principal,$name_moneda,$cod_sunat,$signo,$abreviatura) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Moneda", "Actualizar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
		...
		public function delete($id_moneda) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Moneda", "Eliminar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
