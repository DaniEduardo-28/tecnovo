<?php
	...
	class ClassGaleria extends Conexion {
		...
		public function getDataEditGaleria($id) {
		...
		public function insert($id,$titulo,$descripcion,$estado,$flag_imagen,$src_imagen) {
			...
			try {
				...
				$sql .= ")";
				$stmt = $conexion->prepare($sql);
				$stmt->execute([$titulo,$descripcion,$estado,$src_imagen,"1"]);
				if ($stmt->rowCount()==0) {
		...
		public function show_activos() {
			...
			try {
				...
				$sql = "SELECT g.*
								FROM `tb_galeria` g
								WHERE g.name_tabla = 1 AND g.estado = 'activo'";
