<?php
    ...
    class ClassTipoCosecha extends Conexion {
        ...
        public function insert($descripcion,$estado) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Cosecha", "Insertar"]);
			if ($stmt->rowCount() == 0) {
        ...
        public function update($id_tipo_cosecha,$descripcion,$estado) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Cosecha", "Actualizar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
        ...
        public function delete($id_tipo_cosecha) {
			...
			$sql .= ")";
			$stmt = $conexion->prepare($sql);
			$stmt->execute([$_SESSION['id_trabajador'], "Tipo Cosecha", "Eliminar"]);
			if ($stmt->rowCount() == 0) {
				throw new Exception("Error al realizar el registro en la base de datos.");
			}
