select
    ...
    `d`.`name_documento` AS `name_documento_cliente`
from
    (
        (
            `syscos`.`tb_cliente` `t`
            join `syscos`.`tb_persona` `p` on((`p`.`id_persona` = `t`.`id_persona`))
        )
        join `syscos`.`tb_documento_identidad` `d` on((`d`.`id_documento` = `p`.`id_documento`))
    )