...
innerdivHtml1 += '<th scope="col">Opinión</th>';
  ...
  $.ajax({
    ...
    complete: function (jqXHR, textStatus) {
      ...
      if (totalPaginas==1 && pagina==0) {
		...
		type:"POST",
		url:'ajax.php?accion=showTestimonio'
    ...
    try {
      ...
      if (data1["error"]=="NO") {
        ...
        innerdivHtml += '<th scope="col">Opinión</th>';
...
function saveOperation(){
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        type: "POST",
        url: "ajax.php?accion=goTestimonio",
        contentType: false,
        processData: false,
        data: formdata,
...
function getDataEdit(id){
  $.ajax({
    ...
    url: "ajax.php?accion=getDataEditTestimonio",
    success : function(data) {
      try {
        var data1 = JSON.parse(data);
        if (data1["error"]=="NO") {
          ...
          switch (parseInt(o[0].referencia_1)) {
            case 1:
  ...
  try {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          type: "POST",
          url: "ajax.php?accion=deleteTestimonio",
          datatype: "json",
          data: parametros,
          success: function(data){
            try {
