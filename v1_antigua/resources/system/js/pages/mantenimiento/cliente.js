...
function get_data_callback() {
  ...
  $.ajax({
    ...
    type: "POST",
    url: "ajax.php?accion=showCliente",
  })
      ...
      try {
        ...
        if (data1["error"] == "NO") {
          ...
          for (var i = 0; i < o.length; i++) {
            ...
            innerdivHtml +=
              '<img src="' + o[i].src_imagen + '" alt="" class="img-fluid">';
            innerdivHtml += "</div>";
            ...
            innerdivHtml +=
              '<div class="img-icon"><i class="fa fa-id-card"></i></div>';
            innerdivHtml += "</li>";
            innerdivHtml += '<li class="nav-item">';
            innerdivHtml +=
              "<p>" + o[i].name_documento + " " + o[i].num_documento + "</p>";
            innerdivHtml += "</li>";
            innerdivHtml += "</ul>";
            innerdivHtml += '<ul class="nav">';
            innerdivHtml += '<li class="nav-item">';
            innerdivHtml +=
              '<div class="img-icon"><i class="fa fa-map"></i></div>';
            innerdivHtml += "</li>";
            innerdivHtml += '<li class="nav-item">';
...
function getDataEdit(id_cliente) {
  $.ajax({
    type: "POST",
    data: {
    ...
    url: "ajax.php?accion=getDataEditCliente",
    success: function (data) {
      try {
        var data1 = JSON.parse(data);
        if (data1["error"] == "NO") {
...
function deleteRegistro(id_cliente, cliente) {
  try {
    var parametros = {
      id_cliente: id_cliente,
    };
    ...
    Swal.fire({
      title: "¿Seguro de eliminar el cliente : " + cliente + "?",
      text: "No podrás revertir esta operación.",
      type: "warning",
      showCancelButton: true,
      confirmButtonColor: "#22c63b",
      ...
      if (result.value) {
        $.ajax({
          type: "POST",
          url: "ajax.php?accion=deleteCliente",
          datatype: "json",
          data: parametros,
          success: function (data) {
            try {
