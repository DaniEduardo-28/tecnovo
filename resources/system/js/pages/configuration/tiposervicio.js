...
$(document).ready(function(){
  ...
  $('#example tbody').on( 'click', '#btnDelete', function () {
    try {
      ...
      var data = table.row( $(this).parents('tr') ).data();
      var id_tipo_servicio = data["id_tipo_servicio"];
      ...
      Swal.fire({
        ...
        if (result.value) {
          $.ajax({
            type: "POST",
            url: "ajax.php?accion=deleteTipoServicio",
            datatype: "json",
            data: parametros,
...
function showData(){
  $.ajax({
    url: "ajax.php?accion=showTipoServicio",
    success : function(data) {
      table.clear().draw();
      try {
        ...
        if (data1["error"]=="NO") {
          ...
          for (var i = 0; i < o.length; i++) {
            table.row.add({
              "num": o[i].num,
              "id_tipo_servicio": o[i].id_tipo_servicio,
...
function saveOperation(){
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        type: "POST",
        url: "ajax.php?accion=goTipoServicio",
        datatype: "json",
