...
$(document).ready(function(){
  ...
  $('#tabla_productos tbody').on( 'click', '#btnCheckProducto', function () {
    try {
      ...
      var id_detalle_gastoserv = data["id_detalle_gastoserv"];
      if (num>1) {
        if (verificarproductoontable(id_detalle_gastoserv)==true) {
          generateAlert('warning', 'El Gasto ya se encuentra agregado a la lista.');
          return;
        }
      }
      var descripcion_gasto = data["descripcion_gasto"];
      var monto_gastado= data["monto_gastado"];
      tableForm.row.add({
        "num": num,
        "id_detalle_gastoserv": id_detalle_gastoserv,
