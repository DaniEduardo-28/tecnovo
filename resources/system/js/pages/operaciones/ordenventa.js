...
$(document).ready(function () {
    ...
    try {
      ...
      if (num > 1) {
        if (verificarproductoontable(name_tabla, cod_producto) == true) {
          generateAlert(
            "warning",
            "El Producto ya se encuentra agregado a la lista."
          );
          return;
        }
      }
      ...
      var inputCantidad =
        '<input onkeypress="calcularTotal();" onchange="calcularTotal();" type="number" value="' +
        cantidad +
        '" class="form-control" min="1" style="width:90px;">';
      var inputDescuento =
        '<input onkeypress="calcularTotal();" onchange="calcularTotal();" type="number" value="0" class="form-control" min="0" style="width:90px;">';
      var botonEliminar =
        '<a href="javascript:void(0);" id="botonEliminar" class="btn btn-danger"><i class="fa fa-close"></i></a>';
      ...
      generateAlert(
        "success",
        '<h5 style="text-color:#ffffff">Agregado</h5><br><h6 style="text-color:#f2f9f1">' +
          cantidad +
          " " +
          descripcion +
          " al precio de " +
  ...
  $("#numero_documento_cliente").keypress(function (event) {
    if (event.which == 13) {
      ...
      if ($("#codigo_documento_cliente").val() == "") {
        runAlert(
          "Advertencia",
          "Seleccione un documento de identidad del cliente.",
          "warning"
        );
        return;
      }
      ...
      if ($("#numero_documento_cliente").val() == "") {
        runAlert(
          "Advertencia",
          "Ingrese un número de documento del cliente.",
          "warning"
        );
        return;
      }
  ...
  $("#btnAgregarDetalle").click(function () {
    if ($("#codigo_moneda").val() == "") {
      runAlert(
        "Advertencia",
        "Selecciona la moneda con la que se realizará la operación",
        "warning"
      );
      return;
    }
    ...
    var title =
      "AGREGAR " +
      $("input:radio[name=opcion_busqueda]:checked").val().toUpperCase();
  ...
  $("#btnReportePdf").click(function () {
    ...
    try {
      ...
      var link =
        "?view=reporteventaspdf&id_doc_venta=" +
        id_doc_venta +
        "&id_doc_cliente=" +
        id_doc_cliente +
        "&fecha_inicio=" +
  ...
  $("#btnReporteExcel").click(function () {
    ...
    try {
      ...
      var valor = $("#txtBuscar").val();
      var link =
        "?view=reporteventasexcel&id_doc_venta=" +
        id_doc_venta +
        "&id_doc_cliente=" +
        id_doc_cliente +
        "&fecha_inicio=" +
...
function changeOption() {
  var name_documento = $(
    'select[name="codigo_documento_cliente"] option:selected'
  ).text();
...
function get_data_callback() {
  ...
  $.ajax({
          ...
          for (var i = 0; i < o.length; i++) {
            table.row
              .add({
                ...
                estado: o[i].estado,
                doc_venta:
                  o[i].name_documento_venta +
                  " " +
                  o[i].serie +
                  "-" +
                  o[i].correlativo,
                doc_identidad:
                  o[i].name_documento_cliente +
                  " " +
                  o[i].numero_documento_cliente,
                cliente: o[i].cliente,
    ...
    .fail(function (jqXHR, textStatus, textError) {
      runAlert(
        "Oh No...!!!",
        "Error al realizar la petición " + textError,
        "warning"
      );
    });
}
...
function saveOperation() {
  try {
    ...
    if (
      codigo_documento_venta == "0" ||
      codigo_documento_venta == "" ||
      codigo_documento_venta == 0
    ) {
      runAlert(
        "Faltan Datos",
        "Tiene que seleccionar un tipo de documento.",
        "warning"
      );
      return;
    }
    ...
    if (codigo_documento_cliente == "") {
      runAlert(
        "Faltan Datos",
        "Tiene que seleccionar una documento de cliente.",
        "warning"
      );
      return;
    }
    ...
    if (
      ...
      runAlert(
        "Faltan Datos",
        "Tiene que ingresar el número de documento del cliente.",
        "warning"
      );
      return;
    }
    ...
    if (nombres == "0" || nombres == "" || nombres == 0) {
      runAlert(
        "Faltan Datos",
        "Tiene que ingresar el nombre ó razón social del cliente.",
        "warning"
      );
      return;
    }
    ...
    if (codigo_moneda == "") {
      runAlert(
        "Faltan Datos",
        "Tiene que seleccionar una moneda para realizar la operación.",
        "warning"
      );
      return;
    }
    ...
    if (codigo_forma_pago == "") {
      runAlert(
        "Faltan Datos",
        "Tiene que seleccionar una método de pago realizar la operación.",
        "warning"
      );
      return;
    }
    ...
    if (countRows == 0) {
      runAlert(
        "Faltan Datos",
        "Tiene que tener por lo menos un detalle para guardar la orden de venta.",
        "warning"
      );
      return;
    }
    ...
    if (vuelto < 0) {
      runAlert(
        "Error de Vuelto",
        "El vuelto no puede ser menor a cero.",
        "warning"
      );
      return;
    }
        ...
        if (data1["error"] == "NO") {
          ...
          for (var i = 0; i < o.length; i++) {
            ...
            var inputDescuento =
              '<input onkeypress="calcularTotal();" onchange="calcularTotal();" type="number" value="' +
              descuento +
              '" class="form-control" min="0" style="width:90px;">';
            var botonEliminar =
              '<a href="javascript:void(0);" id="botonEliminar" class="btn btn-danger"><i class="fa fa-close"></i></a>';
      ...
      try {
        ...
        if (data1["error"] == "NO") {
          ...
          for (var i = 0; i < o.length; i++) {
            ...
            var descuento = o[i].detalle_descuento;
            var inputCantidad =
              '<input readonly onkeypress="calcularTotal();" onchange="calcularTotal();" type="number" value="' +
              cantidad +
              '" class="form-control" min="1" style="width:90px;">';
            var inputDescuento =
              '<input readonly onkeypress="calcularTotal();" onchange="calcularTotal();" type="number" value="' +
