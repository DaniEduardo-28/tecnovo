...
function saveOperation(){
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        type: "POST",
        url: "ajax.php?accion=goMedicamento",
        contentType: false,
        processData: false,
        data: formdata,
...
function getDataEdit(id_medicamento){
  $.ajax({
    type: "POST",
    data:{
  		id_medicamento: id_medicamento
		},
    url: "ajax.php?accion=getDataEditMedicamento",
    success : function(data) {
      try {
        var data1 = JSON.parse(data);
        if (data1["error"]=="NO") {
          ...
          var id_tipo_medicamento = o[0].id_tipo_medicamento;
          var flag_encontro = false;
          $("#id_tipo_medicamento option").each(function(){
            if ($(this).val() == id_tipo_medicamento ){
              flag_encontro = true;
            }
          });
          if (!flag_encontro) {
            $('#id_tipo_medicamento').append('<option value="' + id_tipo_medicamento + '" selected>' + name_tipo + '</option>');
          }
          addClassDiv();
        }else {
...
function deleteRegistro(id_medicamento,medicamento){
  ...
  try {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          type: "POST",
          url: "ajax.php?accion=deleteMedicamento",
          datatype: "json",
          data: parametros,
          success: function(data){
            try {
