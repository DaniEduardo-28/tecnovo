...
innerdivHtml1 += '<thead>';
innerdivHtml1 += '<tr>';
innerdivHtml1 += '<th scope="col">Imagen</th>';
  ...
  $.ajax({
    ...
    complete: function (jqXHR, textStatus) {
      ...
      if (totalPaginas==1 && pagina==0) {
		...
		type:"POST",
		url:'ajax.php?accion=showGaleria'
    ...
    try {
      ...
      if (data1["error"]=="NO") {
        ...
        innerdivHtml += '<thead>';
        innerdivHtml += '<tr>';
        innerdivHtml += '<th scope="col">Imagen</th>';
...
function getDataEdit(id){
  $.ajax({
    ...
    url: "ajax.php?accion=getDataEditGaleria",
    success : function(data) {
      try {
        var data1 = JSON.parse(data);
        if (data1["error"]=="NO") {
  ...
  try {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          type: "POST",
          url: "ajax.php?accion=deleteGaleria",
          datatype: "json",
          data: parametros,
          success: function(data){
            try {
